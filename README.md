...
On **Windows**, CI builds a **WinForms launcher** ([`packaging/windows/launcher`](packaging/windows/launcher)) using the **`WinX64SelfContained`** [publish profile](packaging/windows/launcher/Properties/PublishProfiles/WinX64SelfContained.pubxml). **.NET 8** treats **RID-specific** publishes as **framework-dependent by default**, which produced a tiny (~9 MB) single-file that failed before PyApp; the profile forces **`SelfContained` + `PublishSingleFile`** for **win-x64**. The PyApp binary is copied into `packaging/windows/launcher/Embedded/` before `dotnet publish` and stored as an **embedded resource**; at runtime it is written to `%LOCALAPPDATA%\anki-image-updater\pyapp\` when needed. The release artifact is **`AnkiImageUpdater-windows-x64.exe`** (tens of MB); CI sets the icon with [rcedit](https://github.com/electron/rcedit).
