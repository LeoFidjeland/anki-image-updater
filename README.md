...
The **macOS** launcher shows a small **“Preparing the app…”** window with a progress bar while the runtime sets things up. On **Windows**, there is no separate launcher UI: progress appears in the **console window PyApp** opens. Your browser should open automatically shortly after the server starts. Later launches are much faster.
...
The Windows launcher is a small **Rust** binary ([`packaging/windows/launcher-rust`](packaging/windows/launcher-rust)) that extracts embedded PyApp and starts it in a dedicated console; end users do **not** install .NET.
...
On **Windows**, CI copies **`AnkiImageUpdaterPyApp.exe`** into [`packaging/windows/launcher-rust/assets/`](packaging/windows/launcher-rust/assets/) and runs **`cargo build --release`** ([`packaging/windows/launcher-rust`](packaging/windows/launcher-rust)): **embedded PyApp bytes** via `include_bytes!`, **`self restore`** when the payload hash changes, and PyApp started with **`CREATE_NEW_CONSOLE`**. The release artifact is **`AnkiImageUpdater-windows-x64.exe`**; icon and **asInvoker** manifest are embedded at link time via **winres** (`build.rs`). Building on Windows requires **Rust (stable)** and the **MSVC** toolchain.
